...
def preprocess(df):
	...
	df['blkReadInB']  = df['blkReadInB']/df['Runtime']
	df['blkWriteInB'] = df['blkWriteInB']/df['Runtime']
	df['ibReadInB']   = df['ibReadInB']/df['Runtime']
	df['ibWriteInB']  = df['ibWriteInB']/df['Runtime']
	df['llteReadInB'] = df['llteReadInB']/df['Runtime']
	df['llteWriteInB']= df['llteWriteInB']/df['Runtime']
...
def plotAppGrpTrend(df, path):
	...
	if len(list(set(df['JobGrp'].values.tolist()))) != 1:
		...
		filename = str(list(set(df['JobGrp'].values.tolist()))[0])+"PerSec"
...
#	mplt.close()
	...
	if cores is None:
		return
	mplt.plot(cores, blkRead, color = 'g', label="Block IO read", linewidth=outlinewgt[-1])
	mplt.xlabel('Total cores')
	mplt.ylabel('Total bytes/sec')
	plt.tick_params(
	...
	if cores is None:
		return
	mplt.plot(cores, blkWrite, color = 'k', label="Block IO write", linewidth=outlinewgt[-1])
	mplt.xlabel('Total cores')
	mplt.ylabel('Total bytes/sec')
	plt.tick_params(
		axis  ='both',       # changes apply to the x-axis
...
#	mplt.close()
	...
	if cores is None:
		return
	mplt.plot(cores, ibRW, color='b', label="Infiniband data", linewidth=outlinewgt[-1])
	mplt.xlabel('Total cores')
	mplt.ylabel('Total bytes/sec')
	plt.tick_params(
		axis  ='both',       # changes apply to the x-axis
...
#	mplt.close()
	...
	if cores is None:
		return
	mplt.plot(cores, llteRead, color='b', label="Lustre read", linewidth=outlinewgt[-1])
	mplt.xlabel('Total cores')
	mplt.ylabel('Total bytes/sec')
	plt.tick_params(
		axis  ='both',       # changes apply to the x-axis
	...
	if cores is None:
		return
	mplt.plot(cores, llteWrite, color='k', label="Lustre write", linewidth=outlinewgt[-1])
	mplt.xlabel("Total cores")
	mplt.ylabel('Total bytes/sec')
	plt.tick_params(
		axis  ='both',       # changes apply to the x-axis
