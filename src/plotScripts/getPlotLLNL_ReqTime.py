...
if __name__ == "__main__":
	if len(sys.argv) != 7:
		print "Usage ./script.sh <LLNLdatafile> <PathToPutOutputFile> <OutputFilename> <XaxisLabel> <YaxisLabel> <ReqTimeBin(10, 30, 40)>"
		exit()

	df = pd.io.parsers.read_csv(sys.argv[1], sep=',')
	df1 = sliceDF(df, None, int(sys.argv[6]))
	
        # Read the data
        tasks = df1['nodes'].tolist()
